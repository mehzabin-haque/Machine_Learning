...
class Apriori(object):
    ...
    def fit(self, filePath):
        ...
        freqOneTermSet = self.getItemsWithMinSupp(
            transListSet, itemSet, itemCountDict, self.minSupp
        )
        ...
        while currFreqTermSet != set():
            ...
            k += 1
            currCandiItemSet = self.getJoinedItemSet(
                currFreqTermSet, k
            )  # get new candiate k-terms set joining step
            currFreqTermSet = self.getItemsWithMinSupp(
    ...
    def getJoinedItemSet(self, termSet, k):
        ...
        return set(
            [
                term1.union(term2)
                for term1 in termSet
                for term2 in termSet
    ...
    def getItemsWithMinSupp(self, transListSet, itemSet, freqSet, minSupp):
        ...
        for item in itemSet:
            ...
            localSet_[item] += sum(
                [1 for trans in transListSet if item.issubset(trans)]
            )
